...
import {
  ...
  HelpCircle, Link, LockKeyhole, Maximize2, Minimize2, MoreHorizontal, PanelRightClose, PanelRightOpen, Plus, Search, Send, Settings2, Share2,
...
export default function Home() {
  ...
  return <>
    ...
    {reviewOpen && selected && <>{!previewFullscreen && <div className="drag-divider" style={{ right: previewWidth }} role="separator" aria-orientation="vertical" aria-label="Resize preview" onPointerDown={(event) => { event.preventDefault(); setIsResizing(true); }}/>}<aside className={`review-pane ${previewFullscreen ? "preview-fullscreen" : ""}`} style={{ width: previewWidth }}>
      ...
      <div className="review-actions"><div className="version-picker"><button className="version" onClick={() => setVersionMenuOpen((open) => !open)}>v{selectedVersion ?? version}.0 <ChevronDown className={versionMenuOpen ? "flip" : ""} size={14}/></button>{versionMenuOpen && <div className="version-menu">{versions.length ? versions.map((item) => <button key={item.id} className={item.number === selectedVersion ? "active" : ""} onClick={() => { setSelectedVersion(item.number); setVersionMenuOpen(false); }}><span>v{item.number}.0</span><small>{item.message || (language === "zh" ? "初始版本" : "Initial version")}</small></button>) : <span>{language === "zh" ? "暂无版本" : "No versions"}</span>}</div>}</div><button className="view-control" title={commentsCollapsed ? (language === "zh" ? "展开评论" : "Expand comments") : (language === "zh" ? "折叠评论" : "Collapse comments")} onClick={() => setCommentsCollapsed((collapsed) => !collapsed)}>{commentsCollapsed ? <PanelRightOpen size={16}/> : <PanelRightClose size={16}/>}</button><button className="view-control" title={previewFullscreen ? (language === "zh" ? "退出窗口全屏" : "Exit preview fullscreen") : (language === "zh" ? "预览窗口全屏" : "Preview fullscreen")} onClick={() => setPreviewFullscreen((full) => !full)}>{previewFullscreen ? <Minimize2 size={16}/> : <Maximize2 size={16}/>}</button><button onClick={() => setShareOpen(true)}><Share2 size={15}/>{t.share}</button><button className="publish" onClick={publish} disabled={busy === "publish"}><Send size={15}/>{busy === "publish" ? (language === "zh" ? "发布中…" : "Publishing…") : t.publish}</button></div>
      <div className={`review-canvas ${commentsCollapsed ? "comments-collapsed" : ""}`}><ArtifactPreview artifact={selected} version={previewVersion} language={language} onFullscreen={() => setWebFullscreen(true)}/>
    ...
    {webFullscreen && selected?.format === "HTML" && <WebFullscreen title={selected.title} content={previewVersion?.content ?? ""} language={language} onClose={() => setWebFullscreen(false)}/>}
    {toast && <div className="toast"><Sparkles size={16}/>{toast}</div>}
  </>;
}

function ArtifactPreview({ artifact, version, language, onFullscreen }: { artifact: Artifact; version?: ArtifactVersion; language: Language; onFullscreen: () => void }) {
  const content = version?.content ?? "";
  if (artifact.format === "HTML") return <article className="document artifact-preview"><div className="doc-top"><span className="doc-chip"><FileCode2 size={14}/>HTML</span><span>{relativeTime(artifact.updatedAt, language)}</span><button className="web-fullscreen" onClick={onFullscreen}><Maximize2 size={13}/>{language === "zh" ? "网页全屏" : "Fullscreen"}</button></div><iframe title={`${artifact.title} preview`} className="html-preview" sandbox="allow-scripts allow-forms allow-modals allow-popups" srcDoc={content || "<p>No HTML content</p>"}/></article>;
...
function WebFullscreen({ title, content, language, onClose }: { title: string; content: string; language: Language; onClose: () => void }) {
  return <div className="web-preview-overlay" role="dialog" aria-modal="true" aria-label={language === "zh" ? "网页全屏预览" : "Fullscreen web preview"}><header><div><span className="doc-chip"><FileCode2 size={15}/>HTML</span><strong>{title}</strong></div><button onClick={onClose}><Minimize2 size={17}/>{language === "zh" ? "退出全屏" : "Exit fullscreen"}</button></header><iframe title={`${title} fullscreen preview`} sandbox="allow-scripts allow-forms allow-modals allow-popups" srcDoc={content || "<p>No HTML content</p>"}/></div>;
}
